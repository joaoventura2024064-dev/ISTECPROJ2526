...
gantt
    ...
    Protótipos Alta Fidelidade (Figma)      :crit,active ,  b3, after b2, 10d
    
    section Frontend Base
    Configuração React & Ambiente           :done,  b4, 2025-12-05, 5d
    Estrutura Layout & Navegação            :done,  b5, after b4, 5d
    Implementação Auth (Login/Registo)      :done,  b6, after b5, 7d
    Página de Perfil & Dashboard Inicial    :done,           b7, after b6, 1w
    
    section Base de Dados
    Modelação BDDAD (Esquema Relacional)    :done,    b8, 2025-12-10, 5d
    Implementação MySQL (Tables/Keys)       :done,    b9, after b8, 5d
    API Base (Flask Setup)                  :done,    b10, after b9, 5d
    Sprint Review B                         :done,    milestone, 2026-01-10, 0d

    section Simulador e Lógica
    Implementação Modelo SIR (Python)       :done,    c1, 2026-01-10, 7d
    Algoritmos Probabilísticos/Estocásticos :done,    c2, after c1, 5d
    API de Simulação (Endpoints)            :done,    c3, after c2, 5d
    
    section Integração e Testes
    Visualização de Resultados (Gráficos)   :done,  c4, 2026-01-15, 1w
    Histórico de Simulações                 :done,  c5, after c4, 3d
...
| **US_B005** | *Como Programador Frontend, eu pretendo implementar a página de registo de utilizadores, permitindo que novos utilizadores criem uma conta.* | 1 | João | 100% | Completed |
| **US_B006** | *Como Programador Frontend, eu pretendo implementar a página de login para que os utilizadores registados possam aceder à plataforma.* | 1 | João | 100% | Completed |
| **US_B007** | *Como Programador Frontend, eu pretendo desenvolver a página de perfil do utilizador, onde este pode visualizar e atualizar os seus dados pessoais.* | 4 | João | 100% | Completed |
...
| User Story | Descrição | Obrigatoriedade | Responsável | Conclusão | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **US_C001** | *Como Utilizador Registado, eu pretendo aceder a um formulário para configurar os parâmetros de uma simulação (exemplo: a população total, o número inicial de infetados, a taxa de contacto efetivo (β), a taxa de recuperação (γ) e a duração da simulação).* | 1 | João | 100% | Completed |
| **US_C002** | *Como Programador Backend, eu pretendo implementar o motor de simulação que utilize um modelo epidemiológico estocástico do tipo SIR (Suscetíveis-Infetados-Recuperados), onde as transições entre os estados (S→I e I→R) são calculadas com base em distribuições probabilísticas (exemplo: distribuição binomial ou de Poisson), em cada passo de tempo, para refletir a aleatoriedade e os "efeitos de 1º grau" da propagação.* | 1 | Zé | 100% | Completed |
| **US_C003** | *Como Utilizador Registado, eu pretendo poder executar uma simulação com os parâmetros que defini e que o sistema processe o modelo epidemiológico de forma eficiente.* | 1 | Zé | 100% | Completed |
| **US_C004** | *Como Utilizador Registado, eu pretendo que o sistema calcule e apresente o número de reprodução efetivo (Rt) da doença ao longo da simulação, utilizando a fórmula de Von Foerster (Rt = taxa de infeção * duração média da infeção), permitindo compreender a "intensidade em relação à propagação" em diferentes fases da epidemia.* | 3 | Zé | 100% | Completed |
